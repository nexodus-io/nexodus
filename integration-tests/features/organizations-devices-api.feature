Feature: Organization Devices API
  ...
  Scenario: Using the watch option to stream device change events
    ...
    Given I am logged in as "Oliver"
    When I GET path "/api/organizations/${organization_id}/devices?watch=true&gt_revision=0" as a json event stream
    Then the response code should be 200
