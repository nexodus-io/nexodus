...
EOF
    ...
    # Node-1 aircrew run
    cat <<EOF > aircrew-run-node1-cycle2.sh
#!/bin/bash
    aircrew --public-key=${node1_pubkey} \
    --private-key-file=/etc/wireguard/private.key  \
    --controller=${controller} \
...
EOF
    ...
    # Node-2 aircrew run
    cat <<EOF > aircrew-run-node2-cycle2.sh
#!/bin/bash
    aircrew --public-key=${node2_pubkey} \
    --private-key-file=/etc/wireguard/private.key  \
    --controller=${controller} \
...
EOF
    ...
    # STDOUT the run scripts for debugging
    echo "=== Displaying aircrew-run-node1.sh ==="
    cat aircrew-run-node1-cycle2.sh
    echo "=== Displaying aircrew-run-node2.sh ==="
    cat aircrew-run-node2-cycle2.sh

    # Copy files to the containers
    $DOCKER cp ./aircrew-run-node1-cycle2.sh node1:/bin/aircrew-run-node1-cycle2.sh
    $DOCKER cp ./aircrew-run-node2-cycle2.sh node2:/bin/aircrew-run-node2-cycle2.sh
    $DOCKER cp ./node1-private.key node1:/etc/wireguard/private.key
    $DOCKER cp ./node2-private.key node2:/etc/wireguard/private.key

    # Set permissions in the container
    $DOCKER exec node1 chmod +x /bin/aircrew-run-node1-cycle2.sh
    $DOCKER exec node2 chmod +x /bin/aircrew-run-node2-cycle2.sh
    ...
    # Start the agents on both nodes
    $DOCKER exec node1 /bin/aircrew-run-node1-cycle2.sh &
    $DOCKER exec node2 /bin/aircrew-run-node2-cycle2.sh &
...
cycle_configurations(){
    ...
    for i in {1..2}
    do
        cat <<EOF > aircrew-run-node1-cycle${i}.sh
#!/bin/bash
    aircrew --public-key=${node1_pubkey} \
    --private-key-file=/etc/wireguard/private.key \
    --controller=${controller} \
...
EOF
        ...
        cat <<EOF > aircrew-run-node2-cycle${i}.sh
#!/bin/bash
    aircrew --public-key=${node2_pubkey} \
    --private-key-file=/etc/wireguard/private.key \
    --controller=${controller} \
...
EOF
        ...
        cat <<EOF > aircrew-run-node3-cycle${i}.sh
#!/bin/bash
    aircrew --public-key=${node3_pubkey} \
    --private-key-file=/etc/wireguard/private.key \
    --controller=${controller} \
...
EOF
    ...
    # Copy files to the containers
    sudo $DOCKER cp ./aircrew-run-node1-cycle1.sh node1:/bin/aircrew-run-node1-cycle1.sh
    sudo $DOCKER cp ./aircrew-run-node2-cycle1.sh node2:/bin/aircrew-run-node2-cycle1.sh
    sudo $DOCKER cp ./aircrew-run-node3-cycle1.sh node3:/bin/aircrew-run-node3-cycle1.sh

    # Set permissions in the container
    sudo $DOCKER exec node1 chmod +x /bin/aircrew-run-node1-cycle1.sh
    sudo $DOCKER exec node2 chmod +x /bin/aircrew-run-node2-cycle1.sh
    sudo $DOCKER exec node3 chmod +x /bin/aircrew-run-node3-cycle1.sh

    # Start the agents on all 3 nodes nodes (currently the hub-router needs to be spun up first)
    sudo $DOCKER exec node1 /bin/aircrew-run-node1-cycle1.sh &
    sudo $DOCKER exec node2 /bin/aircrew-run-node2-cycle1.sh &
    sudo $DOCKER exec node3 /bin/aircrew-run-node3-cycle1.sh &
    ...
    sudo $DOCKER exec node1 killall aircrew
    sudo $DOCKER exec node2 killall aircrew
    sudo $DOCKER exec node3 killall aircrew
    ...
    # Copy files to the containers
    sudo $DOCKER cp ./aircrew-run-node1-cycle2.sh node1:/bin/aircrew-run-node1-cycle2.sh
    sudo $DOCKER cp ./aircrew-run-node2-cycle2.sh node2:/bin/aircrew-run-node2-cycle2.sh
    sudo $DOCKER cp ./aircrew-run-node3-cycle2.sh node3:/bin/aircrew-run-node3-cycle2.sh

    # Set permissions in the container
    sudo $DOCKER exec node1 chmod +x /bin/aircrew-run-node1-cycle2.sh
    sudo $DOCKER exec node2 chmod +x /bin/aircrew-run-node2-cycle2.sh
    sudo $DOCKER exec node3 chmod +x /bin/aircrew-run-node3-cycle2.sh

    # Start the agents on all 3 nodes nodes (currently the hub-router needs to be spun up first)
    sudo $DOCKER exec node1 /bin/aircrew-run-node1-cycle2.sh &
    sudo $DOCKER exec node2 /bin/aircrew-run-node2-cycle2.sh &
    sudo $DOCKER exec node3 /bin/aircrew-run-node3-cycle2.sh &
