...
func (api *API) CreateRegKey(c *gin.Context) {
	...
	err = api.transaction(ctx, func(tx *gorm.DB) error {
		...
		if request.SecurityGroupId != nil {
			...
			record.SecurityGroupId = request.SecurityGroupId
		}
...
// @Param        update body       models.UpdateRegKey true "RegKey Update"
...
// @Failure      429  {object}     models.BaseError
// @Failure      500  {object}  models.InternalServerError "Internal Server Error"
// @Router       /api/reg-keys/{id} [patch]
func (api *API) UpdateRegKey(c *gin.Context) {
	ctx, span := tracer.Start(c.Request.Context(), "UpdateRegKey", trace.WithAttributes(
	...
	var request models.UpdateRegKey
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, models.NewBadPayloadError(err))
		return
	}

	var regKey models.RegKey
	err = api.transaction(ctx, func(tx *gorm.DB) error {

		result := api.RegKeyIsForCurrentUser(c, tx).
			First(&regKey, "id = ?", k)
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return NewApiResponseError(http.StatusNotFound, models.NewNotFoundError("reg key"))
		}
		...
		if request.SecurityGroupId != nil {
			...
			regKey.SecurityGroupId = request.SecurityGroupId
		}
		if request.Description != nil {
			regKey.Description = *request.Description
		}
		if request.ExpiresAt != nil {
			zero := time.Time{}
			if *request.ExpiresAt == zero {
				regKey.ExpiresAt = nil
			} else {
