...
func (suite *HandlerTestSuite) TestCreateDuplicatedOrganizationConflict() {
	assert := suite.Assert()
	require := suite.Require()
	...
	// Create the organization once and expect a success response
	{
	...
	// Recreate the organization and expect a conflict error
	{
		reqBody, err := json.Marshal(organization)
		require.NoError(err)
		_, res, err := suite.ServeRequest(
		...
		// require.Error(err)
		// require.NoError(err)
		require.Equal(http.StatusConflict, res.Code, string(body))

		// assert.Error(err)
		// require.ErrorContains(err, "409")
