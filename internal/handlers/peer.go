...
var rollback = errors.New("rollback")
...
func (api *API) CreatePeerInZone(c *gin.Context) {
	...
	var peer *models.Peer
	err = api.transaction(ctx, api.db, nil, func(tx *gorm.DB) error {
		...
		if found {
			...
			peer.EndpointLocalAddressIPv4 = request.EndpointLocalAddressIPv4
			...
			err = tx.Create(peer).Error
			if err != nil {
				return err
			}
			zone.Peers = append(zone.Peers, peer)
			err = tx.Save(&zone).Error
			if err != nil {
				return err
			}
	...
	if err != nil {
		if errors.Is(err, rollback) {
			// original error has already been logged, and http status set.
			return
		}
