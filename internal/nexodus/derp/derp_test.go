...
func TestSendRecv(t *testing.T) {
	...
	wantActive(3, 0)
	_ = clients[0].NotePreferred(true)
	wantActive(3, 1)
	_ = clients[0].NotePreferred(true)
	wantActive(3, 1)
	_ = clients[0].NotePreferred(false)
	wantActive(3, 0)
	_ = clients[0].NotePreferred(false)
	wantActive(3, 0)
	_ = clients[1].NotePreferred(true)
	wantActive(3, 1)
	connsOut[1].Close()
	wantActive(2, 0)
	wantClosedPeers(1)
	_ = clients[2].NotePreferred(true)
	wantActive(2, 1)
	_ = clients[2].NotePreferred(false)
	wantActive(2, 0)
	connsOut[2].Close()
	wantActive(1, 0)
	wantClosedPeers(1)
...
func TestSendFreeze(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			_ = aliceClient.Send(cathyKey.Public(), msg2)
		}
	}()
...
func TestMultiForwarder(t *testing.T) {
	...
	for i := 0; i < numMsgs; i++ {
		...
		fwd = s.clientsMesh[u]
		s.mu.Unlock()
		_ = fwd.ForwardPacket(u, u, []byte(strconv.Itoa(i)))
	}
