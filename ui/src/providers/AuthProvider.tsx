...
export const goOidcAgentAuthProvider = (api: string): AuthProvider => ({
  login: async (params = {}) => {
    ...
    if (!params.code || !params.state) {
      const request = new Request(`${api}/browser/login/start`, {
        method: "POST",
        credentials: "include",
      });
      try {
    ...
    const request = new Request(`${api}/browser/login/end`, {
      method: "POST",
      credentials: "include",
  ...
  logout: async () => {
    const request = new Request(`${api}/browser/logout`, {
      method: "post",
      credentials: "include",
    });
    try {
  ...
  getIdentity: async (): Promise<UserIdentity> => {
    const request = new Request(`${api}/browser/user_info`, {
      credentials: "include",
    });
    var id;
    try {
